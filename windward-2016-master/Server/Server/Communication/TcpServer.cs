...
namespace Server.Communication
{
	public class TcpServer
	{
		...
		// BUGBUG - increase SECONDS_WAIT_TURN if you want the server to wait longer while you're debugging the client.
		private const int SECONDS_WAIT_TURN = 2;
		private Socket serverSocket;
		private IEngineCallback engineCallback;
		private readonly List<ConnectionInfo> connections = new List<ConnectionInfo>();

		private static readonly Dictionary<string, XDocument> replyMessages = new Dictionary<string, XDocument>();
		...
		private void SetupServerSocket()
		{
			...
			serverSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp) { Blocking = false, ReceiveBufferSize = BUFFER_SIZE, SendBufferSize = BUFFER_SIZE };
			connections.Clear();
		...
		private void ReceiveCallback(IAsyncResult result)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					// reply message we save off
					XDocument xml = XDocument.Parse(message);
					if (xml.Root.Name == "reply")
					{
						replyMessages.Add(xml.Root.Attribute("msg-id").Value, xml);
						continue;
					}
		...
		public static XDocument GetReply(string msgId)
		{
			// polling is poor form, but we've got nothing else to do while we wait and it's fast (usually).
			DateTime timeout = DateTime.UtcNow.AddSeconds(SECONDS_WAIT_TURN);
			while (timeout > DateTime.UtcNow)
			{
				if (replyMessages.ContainsKey(msgId))
				{
					XDocument xml = replyMessages[msgId];
					replyMessages.Remove(msgId);
					return xml;
				}
				// do NOT call Application.DoEvents() here!!!
				Thread.Sleep(5);
			}
			return null;
		}
